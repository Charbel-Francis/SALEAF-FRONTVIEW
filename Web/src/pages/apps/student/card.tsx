...
import StudentCard from 'sections/apps/student/StudentCard';
...
function dataSort(data: StudentList[], sortBy: string) {
...
export default function StudentCardPage() {
  ...
  const { students: lists } = useGetStudent();

  const [sortBy, setSortBy] = useState('Default');
  const [globalFilter, setGlobalFilter] = useState('');
  const [userCard, setUserCard] = useState<StudentList[]>([]);
  ...
  return (
    <>
        ...
        <Stack direction="row" alignItems="center">
          ...
          >
            ...
            <Stack direction={matchDownSM ? 'column' : 'row'} alignItems="center" spacing={1}>
              ...
              <Button variant="contained" onClick={() => setStudentModal(true)} size="large" startIcon={<Add />}>
                Add Student
              </Button>
            </Stack>
          </Stack>
      ...
      <Grid container spacing={3}>
        {!studentLoading && userCard.length > 0 ? (
          _DATA.currentData().map((user: StudentList, index: number) => (
          ...
          <EmptyUserCard title={studentLoading ? 'Loading...' : 'You have not created any student yet.'} />
        )}
      </Grid>
      ...
      <StudentModal open={studentModal} modalToggler={setStudentModal} />
    </>
  );
}
